interface SoulProfile {
  name: string;
  description: string;
  system_modifier: string;
  max_position_pct: number;
  stop_loss_pct: number;
  preferred_assets: string[];
}

export const SOULS: Record<string, SoulProfile> = {
  conservative: {
...
export const DEFAULT_SOUL = "balanced";

export class Soul {
  private _soulId: string;

  constructor(soulId = DEFAULT_SOUL) {
    if (!(soulId in SOULS)) {
      throw new Error(`Unknown soul: ${soulId}. Available: ${Object.keys(SOULS).join(", ")}`);
    }
    this._soulId = soulId;
  }
  ...
  get profile(): SoulProfile {
    return SOULS[this._soulId];
  }
  ...
  get systemModifier(): string {
    return this.profile.system_modifier;
  }

  switch(soulId: string): void {
    if (!(soulId in SOULS)) {
      throw new Error(`Unknown soul: ${soulId}. Available: ${Object.keys(SOULS).join(", ")}`);
    }
    this._soulId = soulId;
  ...
  static listSouls(): Array<{ id: string; name: string; description: string }> {
    return Object.entries(SOULS).map(([k, v]) => ({
      id: k,
      name: v.name,
